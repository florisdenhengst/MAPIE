...
import warnings
from typing import Any, Iterable, Optional, Tuple, Union, cast, Dict
...
class HierarchicalMapieClassifier(BaseEstimator, ClassifierMixin):
    """
    Prediction sets for hierarchical classification.
    ...
    def __init__(
        ...
        verbose: int = 0,
        costs_per_node: Optional[ArrayLike] = None,
        clip: Optional[Tuple[float, float]] = None,
        cp_method: Optional[str] = 'lac',
    ) -> None:
        ...
        self.costs_per_node = costs_per_node
        self.clip = clip
        self.cp_method = self.cp_approach_map[cp_method]
    ...
    def _check_fit_parameter(
        ...
        return (
            estimator, cv, X, y, y_enc, sample_weight, groups
        )
    ...
    def fit(
        ...
        **kwargs: Any
        ...
        costs_per_node: Optional[Dict[int, float]]
            Mapping of node IDs to costs to minimize during inference
        ...
        self.conformity_score_function = self.cp_method()
        # self.conformity_score_function.classes = self.classes # only required for APS
        self.conformity_score_function.random_state = self.random_state
        self.y_average = y.sum(axis=1).mean()
        ...
        y_pred_proba = self.estimator_.predict_proba(
            X
        )
            
        # Compute the conformity scores
        # self.conformity_score_function_.set_ref_predictor(self.estimator_)
        self.conformity_scores_ =  self.conformity_score_function.get_hierarchical_conformity_scores(
                y, y_pred_proba, y_enc=y_enc, X=X,
                sample_weight=sample_weight, groups=groups,
                method=self.method,
                costs=self.costs_per_node,
                clip=self.clip,
            )
        return self
    ...
    def predict(
        ...
        **predict_params
        ...
        prediction_sets = self.conformity_score_function.predict_set(
            X, alpha_np,
        ...
        self.quantiles_ = self.conformity_score_function.quantiles_
